...
**65 已完成**（七类重复各收成一份）；本地 `main` 领先 `origin/main` 若干提交
（写这份交接时是 10 个），**这些改动都还没上线**。下一步从 **64（PNG 的 CRC 真校验）** 开始。

| 项 | 值 |
| --- | --- |
| 本地 `main` HEAD | 看 `git log -1 --oneline`；`git log --oneline origin/main..HEAD` 列出所有**未推**的提交（数字每次提交都会变，别照抄） |
| 线上网页 | 还是 `index-JFtj61pq.js`（2026-09-24 23:16 那次部署）；**63 / 66 / 65 都还没上线** |
...
1. **64**（PNG 的 CRC 真校验）：遍历 PNG 时真的算 `crc32`，不匹配的块跳过并发 `png.bad-crc`
   warning；单测构造一个坏块，让注释与行为对齐。
2. 然后 **67**（管理员小修）：`admin.error` 传给 `SideChat` 显示、工具注释与文案改成五个、
   未知参数回填「未识别的参数：…」。
3. 报告节奏：**每完成 5 个任务向用户做一次完整汇报**（含实测数字与上线状态）。到这一批
   （63 / 66 / 65 / 64 / 67）收齐时正好一次；**上线与推送要问用户**
