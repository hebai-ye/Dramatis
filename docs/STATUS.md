...
刚在 `e140839` 提交并上线的那个）删掉了，换成对话级勾选框**「无限制模式」**；提示词正文移到代码里的
**一处常量** `packages/core/src/prompt/unlimited.ts` 的 `UNLIMITED_MODE_PROMPT`
（**当前是空的，等用户粘贴**）。打开后整块进 system 提示（`droppable: false`，与角色卡系统提示同档，
预算紧张时不会被静默省略）。旧字段 `modes.advancedSystemPrompt` **照旧装配**、不静默丢弃：
菜单里会显示它还在生效并给一个「清空旧提示」按钮——**线上现在还是旧界面**，
所以用户在线上的旧库里可能已经存了内容，这条兼容是必须的。单测 +6；另外**临时把常量填上文字
跑过一次**，确认真的进了 `messages[0]`（跑完已还原）。真实模型上的效果、真机菜单观感**待 Codex 验**；
粘贴位置也写进了 [ROLEPLAY-PROMPT.md](./ROLEPLAY-PROMPT.md)。**本批未提交、未 push、未上线**。
