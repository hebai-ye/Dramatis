...
Dramatis 是一个多角色扮演酒馆，兼容 SillyTavern 资产格式。**P0（9 项）全部完成，P1 完成 10/11；界面改版三批（A/B/C）全部完成；P1-10 七轮真模型验证跑完（含 T4 五十回合长跑、T2c 生成前意图、T3 分层摘要）；用量账单已落盘（T7）**，315 个测试通过。

**下一批做什么看 [TASKS.md](./TASKS.md)**：T3 剩下的一半（记忆的「合并成粗粒度印象」）、
P1-11 向量召回（长跑已经跑完两轮，可以评估关键词够不够用了）、P1-9 的预算熔断
（真实单价与每回合成本都齐了，可以设阈值了）。
P1 只剩 P1-9 的熔断部分与 P1-11 向量检索——两者现在都有数据支撑。
...
**P1 剩余**：P1-9 的预算熔断（批量合并已随 T2c 落地，成本数据已由 T7 补齐，缺的是阈值决策）、P1-11 向量检索（前置条件 P1-10 已满足，可以评估关键词召回够不够用）。

**已完成**：P1-1~P1-4 记忆、P1-5 分层摘要（T3，遗忘机制另半留下）、P1-6 意图先行调度（T2c）、P1-7/P1-8 情绪与抗漂移、P1-10 七轮真模型验证。
