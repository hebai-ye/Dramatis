...
Dramatis 是一个多角色扮演酒馆，兼容 SillyTavern 资产格式。**P0（9 项）全部完成，P1 完成 7/11**，176 个测试通过。尚未用真实模型验证过输出质量。
...
**下一批**：布局改版。用户提供了手绘草图，方向已明确——顶栏收窄、两栏布局、左栏分头/身/脚三段、主区有自己的标题栏。两点待确认：主区右侧那条竖线是第三条窄栏还是草稿纸格线；运行时面板是否默认折叠、由主区标题栏的按钮开关。
