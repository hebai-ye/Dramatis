...
**需要人工做的**：P1-10 的真实模型验证。**准备已经做完了**——一条命令打印出代码
真正会发出去的四条提示词（主对话 / 记忆抽取 / 情绪推演 / 世界管理员 + 工具声明），
回答贴回 `packages/core/src/eval/real-model-responses.ts` 就会变成可重跑的解析断言。
步骤与判定标准见 EVAL.md 第二节。

卡在哪：应用内浏览器里的 DeepSeek 网页版是未登录状态（登录要短信验证码或扫码，
必须本人操作）；通过扩展连接的 Edge 在当前环境里驱动不了（工具直接报
`unsupported Codex auth method`）；本机也没有在跑的本地模型（11434 / 1234 都不通）。
所以这一步要么在浏览器里登录一次，要么给一个任意 OpenAI 兼容的 API Key。
