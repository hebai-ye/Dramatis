...
**一句话**：57–63 已落地，**66 已完成**（四个 hook 全搬完，App.tsx 1947 → 1090 行）；本地
`main` 领先 `origin/main` 若干提交（写这份交接时是 8 个，含 63 与 66 的三步），
**这些改动都还没上线**。下一步从 **65（代码重复收敛）** 开始。

| 项 | 值 |
| --- | --- |
| 本地 `main` HEAD | 看 `git log -1 --oneline`；`git log --oneline origin/main..HEAD` 列出所有**未推**的提交（数字每次提交都会变，别照抄） |
| 线上网页 | 还是 `index-JFtj61pq.js`（2026-09-24 23:16 那次部署）；**63 与 66 的三步都还没上线** |
...
1. **65**（代码重复收敛）：`asRecord` 7 份、`formatTime` 4 份、composer 两份、记账/入队各三份、
   模态框三个。无行为改变，做完跑一遍界面回归。
2. 然后 **64**（PNG 的 CRC 真校验）→ **67**（管理员小修）。
