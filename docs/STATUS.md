...
Dramatis 是一个多角色扮演酒馆，兼容 SillyTavern 资产格式。**P0（9 项）全部完成，P1 全部收口（11/11，其中 P1-11 以评测结论收口：证据不支持上向量）；界面改版三批（A/B/C）全部完成；P1-10 七轮真模型验证 + 三次评测回归跑完；用量账单（T7）、调用熔断（T19）、分层摘要（T3）都已落地**，330 个测试通过。

**下一批做什么看 [TASKS.md](./TASKS.md)**：T22 同类条目去重（评测指出的真问题：预算一半被
次相关条目占掉）、T3 剩下的一半（记忆合并成粗粒度印象），以及 P2 的多端
（响应式 / PWA / 账号同步）。
