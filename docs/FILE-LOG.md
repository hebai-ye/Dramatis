...
> 重点部分见第一节（各 `.md` 文档的用途）。
>
> 生成时间：2026-09-16 23:05；**最新一批改动见第六节末尾**（T1 动作分段、T2a 追问延续、
> 设置面板草稿式保存，提交 `f4e2974`）。下方时间表是 23:05 的快照，
> 之后再改过的文件以「最近提交」为准。
...
整个项目是在同一天里从零搭起来并迭代出来的，所以按时间读提交历史就等于读一份开发日志。
...
| 23:1x | `f4e2974` | T1 动作分段（引号即对白）、T2a 追问延续、设置面板草稿式保存 |
