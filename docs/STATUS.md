...
Dramatis 是一个多角色扮演酒馆，兼容 SillyTavern 资产格式。**P0（9 项）全部完成，P1 完成 8/11；界面改版三批（A/B/C）全部完成；P1-10 四轮真模型验证跑完，含 T4 五十回合长跑**，253 个测试通过。

**下一批做什么看 [TASKS.md](./TASKS.md)**：语音归属错位（T18，长跑里最影响观感的问题）、
换场带谁（T17）、意图先行调度的剩余部分、成本统计持久化（T7）。
