...
Dramatis 是一个多角色扮演酒馆，兼容 SillyTavern 资产格式。**P0（9 项）全部完成，P1 完成 8/11；界面改版三批（A/B/C）全部完成；P1-10 五轮真模型验证跑完，含 T4 五十回合长跑**，275 个测试通过。

**下一批做什么看 [TASKS.md](./TASKS.md)**：T2c 生成前意图（方案与算法都写好了：合并抽取与
推演腾出预算）、成本统计持久化（T7）、P1-5 分层摘要（T3，50 回合长跑已给出依据）。
...
**P1-10 的真实模型验证已经跑完四轮（2026-09-16，DeepSeek）**，完整记录见
[EVAL.md](./EVAL.md) 第二节：
