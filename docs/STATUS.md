...
Dramatis 是一个多角色扮演酒馆，兼容 SillyTavern 资产格式。**P0（9 项）全部完成，P1 完成 7/11；界面改版三批（A/B/C）全部完成；P1-10 真模型验证三轮跑完，动作分段与追问延续都已真机验证**，251 个测试通过。

**下一批做什么看 [TASKS.md](./TASKS.md)**：意图先行调度的剩余部分、50 回合长跑、成本统计完整化。
...
**P1-10 的真实模型验证已经跑完两轮（2026-09-16，DeepSeek）**，完整记录见
[EVAL.md](./EVAL.md) 第二节：
...
**仍然缺的**：真实 token 账单（provider 默认不取 `usage`，界面也没展示，属于 P3-7），
以及 `#` 动作约定的模型遵守率只有约 2/12——这两个是 P1-9 熔断与提示词下一轮的主要输入。
50 回合的长跑（长线不崩、性格不趋同）也还没跑过。
