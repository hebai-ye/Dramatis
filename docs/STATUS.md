...
Dramatis 是一个多角色扮演酒馆，兼容 SillyTavern 资产格式。**P0（9 项）全部完成，P1 完成 9/11；界面改版三批（A/B/C）全部完成；P1-10 六轮真模型验证跑完，含 T4 五十回合长跑与 T2c 生成前意图；用量账单已落盘（T7）**，299 个测试通过。

**下一批做什么看 [TASKS.md](./TASKS.md)**：P1-5 分层摘要（T3，50 回合长跑已给出依据）、
T18 更根本的错位治理（模型侧）、P1-9 的预算熔断（真实单价已经有了，可以设阈值了）。
P1 只剩 P1-5 分层摘要、P1-9 的熔断部分（数据已就位，阈值未定）与 P1-11 向量检索。
