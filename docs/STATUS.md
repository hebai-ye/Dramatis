...
**P1-10 的第一轮已经跑完（2026-09-16，DeepSeek 网页版）**：四条提示词（主对话 /
记忆抽取 / 情绪推演 / 世界管理员 + 工具声明）都贴过一遍，回答原样记在
`packages/core/src/eval/real-model-responses.ts`，并由第二条用例跑过
`renderMessageContent` / `parseExtraction` / `parseAffectUpdates` / `parseAdminToolCall`。

结论与三处修复见 [EVAL.md](./EVAL.md) 第二节：格式与视角都成立（动作 `#` 分段、
两个角色的记忆各说各话、关系变化不超上限），但**改世界书丢原有设定**、
**`set_scene` 顺手改入场策略**这两个问题被当场抓出来并改掉了，复验通过。

**还差的**：端到端长跑（真的聊十几轮、看着记忆累积）需要自备一个 OpenAI 兼容的
API Key——网页版没有 chat 角色区分，也不能真的执行工具调用。P1-11 因此仍然
只能算完成一半前置条件。
