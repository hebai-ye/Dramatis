...
Dramatis 是一个多角色扮演酒馆，兼容 SillyTavern 资产格式。**P0（9 项）全部完成，P1 全部收口（11/11，P1-11 以评测结论收口：证据不支持上向量）；界面改版三批（A/B/C）全部完成；P1-10 七轮真模型验证 + 五次评测回归跑完；用量账单（T7）、调用熔断（T19）、分层摘要（T3）、记忆预算的兜底上限（T22）、封存导出（T9）都已落地**，351 个测试通过。

**下一批做什么看 [TASKS.md](./TASKS.md)**：T3 剩下的一半（记忆合并成粗粒度印象）、
以及 P2 的多端（T8 响应式 / PWA、账号与同步）——封存导出（T9）已交付，
它是同步做出来之前的数据安全网。
