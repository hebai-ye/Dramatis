...
Dramatis 是一个多角色扮演酒馆，兼容 SillyTavern 资产格式。**P0（9 项）全部完成，P1 完成 10/11（只剩条件触发的向量召回）；界面改版三批（A/B/C）全部完成；P1-10 七轮真模型验证跑完（含 T4 五十回合长跑、T2c 生成前意图、T3 分层摘要）；用量账单已落盘（T7）、调用熔断已就位（T19）**，329 个测试通过。

**下一批做什么看 [TASKS.md](./TASKS.md)**：T3 剩下的一半（记忆的「合并成粗粒度印象」）、
P1-11 向量召回（两轮长跑的数据都在，可以评估关键词召回够不够用）、
以及 P2 的多端（响应式 / PWA / 账号同步）。P1 只剩 P1-11 这一项条件任务。
...
**P1 剩余**：只剩 P1-11 向量检索（条件触发：长跑已跑完两轮，可以评估关键词召回够不够用）。
P1-9 的批量合并（T2c）与预算熔断（T19）都已落地。
