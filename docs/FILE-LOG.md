...
| 文档 | 创建 | 修改 | 用途 |
| --- | --- | --- | --- |
| [`README.md`](../README.md) | 2026-09-16 00:05 | 2026-09-16 22:04 | **项目门面**。给第一次看到这个仓库的人：一句话定位、与 SillyTavern 的关系、已经能用的功能清单、桌面入口与快速开始、核心设计摘要、路线图状态。也承担英文 TL;DR 与关键词，便于被搜索到 |
...
| [`docs/ROADMAP.md`](ROADMAP.md) | 2026-09-16 11:10 | 2026-09-16 22:42 | **执行计划**。P0–P3 全部任务、依赖关系、执行顺序、风险登记册、验收标准、任务状态表。每个批次的「实现说明」也写在这里（踩过的坑、与计划的偏差） |
| [`docs/LAYOUT.md`](LAYOUT.md) | 2026-09-16 21:26 | 2026-09-16 22:05 | **界面与交互规格**。来自手绘草图与逐区域填写，是界面实现的唯一依据；含「与当前实现的差异」对照表、三批进度、以及**实现时的取舍**（哪些细节规格没写、实现时怎么定的） |
| [`docs/STATUS.md`](STATUS.md) | 2026-09-16 20:49 | 2026-09-16 22:42 | **接续点**。给下一个会话（或下一个人）：一句话状态、已完成批次、不可回退的设计决定、待办与卡点、怎么继续跑。**新会话应当先读它** |
| [`docs/EVAL.md`](EVAL.md) | 2026-09-16 18:46 | 2026-09-16 22:41 | **评测与验证**。上半部分是自动化基线（脚本化模型的 50 回合长跑与压力场景及其结论），下半部分是真模型人工验证：怎么生成提示词、判定标准、两轮验证记录与发现的问题 |
| [`docs/TASKS.md`](TASKS.md) | 2026-09-16 23:04 | 2026-09-16 23:04 | **工作清单**（本次新增）。回答「下一批动手做什么」：P1 收尾、验证任务、P2 功能添加、真实使用中发现的优化项，各带来源、验收标准与建议顺序。与 ROADMAP 的分工写在文首 |
| [`docs/FILE-LOG.md`](FILE-LOG.md) | 2026-09-16 23:05 | 2026-09-16 23:05 | **本文档**（本次新增）。文件级时间日志：每个文件的创建/修改/提交时间，加上各 md 文档的用途索引 |
...
| 09-19 17:2x | `T2c` | 生成前意图调用 + 抽取与推演合并成一次后台调用（提交见下） |
...
| `packages/core/src/director/intent-plan.ts` | 2026-09-19 17:13 | 2026-09-19 17:18 | 未提交（T2c） | **T2c 生成前意图**：导演提示词、宽松解析、三道闸挑人 |
| `packages/core/src/director/intent-plan.test.ts` | 2026-09-19 17:13 | 2026-09-19 17:13 | 未提交（T2c） | 9 条：四种 mode、名字对不上、不在场、hold_back |
| `packages/core/src/memory/turn-analysis.ts` | 2026-09-19 17:12 | 2026-09-19 17:18 | 未提交（T2c） | **T2c 预算来源**：抽取 + 推演合并成一次调用 |
| `packages/core/src/memory/turn-analysis.test.ts` | 2026-09-19 17:13 | 2026-09-19 17:18 | 未提交（T2c） | 5 条：合并形状、嵌套形状、只写一半 |
...
| `packages/core/src/model/message.ts` | 2026-09-19 17:18 | `intentSource` 增加 `planned`（未提交） |
| `packages/core/src/model/conversation.ts` | 2026-09-19 17:14 | 会话模式增加 `intentFirst`（缺省开，未提交） |
| `packages/core/src/prompt/assemble.ts` | 2026-09-19 17:14 | 把这一轮的打算写进「本轮指令」（未提交） |
| `packages/core/src/index.ts` | 2026-09-19 17:18 | 导出新模块（未提交） |
| `apps/web/src/App.tsx` | 2026-09-19 17:21 | 生成前意图调用、合并入队、失败静默退回（未提交） |
| `apps/web/src/components/MainChat.tsx` | 2026-09-19 17:18 | 「他这一轮想：」chip、模式菜单加「意图先行」（未提交） |
| `apps/web/src/components/MemoryPanel.tsx` | 2026-09-19 17:23 | 「后台调用」改称「额外调用」（未提交） |
| `apps/web/src/lib/worker.ts` | 2026-09-19 17:23 | `turn.analyze` 合并任务 + `recordUsage`（未提交） |
